...
func main() {
	...
	router.HandleFunc("/api/create/task", c.AddTask).Methods("POST", "OPTIONS")
	router.HandleFunc("/api/task/{id}", c.GetTask).Methods("GET")
	router.HandleFunc("/api/edit/task/{id}", c.UpdateTask).Methods("POST", "OPTIONS")
	router.HandleFunc("/api/delete/task/{id}", c.DeleteTask).Methods("GET")
